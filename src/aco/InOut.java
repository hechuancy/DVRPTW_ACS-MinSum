...
public class InOut {
    ...
    static double node_branching(double l) {
        ...
        for (m = 0; m < (VRPTW.n + 1); m++) {
            /* determine max, min to calculate the cutoff value */   //截断值
    ...
    // 对两个double数组，进行同位置的元素相减，返回一个等长的数组
    public static double[] addDoubleArray(double[] arr1, double[] arr2) {
        if (arr1.length != arr2.length) {
            throw new ValueException("Error！两个输入数组长度不等！");
        }
    ...
    // 对两个double数组，进行同位置的元素相减，返回一个等长的数组
    public static double[] minusDoubleArray(double[] arr1, double[] arr2) {
        if (arr1.length != arr2.length) {
            throw new ValueException("Error！两个输入数组长度不等！");
        }
        double[] c = new double[arr1.length];
    ...
    // 判断两个doubel数组，是否按照元素严格小于等于，主要用于车辆当前使用的需求数组，是否严格小于等于车辆的容量数组
    public static boolean compareArrayALessEqualArrayB(double[] arr1, double[] arr2) throws ValueException {
        if (arr1.length != arr2.length) {
            throw new ValueException("Error！两个输入数组长度不等！");
        }
    ...
    // 将二维的容量数组/需求数组转化未字符串形式，以供打印输出
    public static String toStringFromQuantityArray(ArrayList<double[]> currentQuantityArray) {
        StringBuilder s = new StringBuilder(new String());
        s.append("[");
        int i = 0;
        for (double[] arr : currentQuantityArray) {
            s.append(Arrays.toString(arr));
            if (i < currentQuantityArray.size()-1) {
                s.append(", ");
            }
            i += 1;
        }
