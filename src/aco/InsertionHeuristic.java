...
import static aco.InOut.compareArrayALessEqualArrayB;
...
//        currentQuantity = reqList.get(0).getDemand();
        currentQuantityArray = reqList.get(0).getDemandArray();
...
//            currentQuantity += reqList.get(currentCity + 1).getDemand();
            currentQuantityArray = addDoubleArray(currentQuantityArray, reqList.get(currentCity + 1).getDemandArray());
...
//            isFeasible = false;
    ...
    static boolean isFeasibleInsertion(Ant a, VRPTW vrp, int customer, int indexTour, int previousPos, int nextPos) {
        ...
        double[] currentQuantityArray;  // 当前的容量数组
...
//    	currentQuantity = a.currentQuantity.get(indexTour) + reqList.get(customer + 1).getDemand();
        currentQuantityArray = addDoubleArray(a.currentQuantityArray.get(indexTour), reqList.get(customer + 1).getDemandArray());
...
//		}
        ...
        if ((InOut.compareArrayALessEqualArrayB(currentQuantityArray, vrp.getCapacityArray())) && (earliestTime <= latestTime) && (beginService <= reqList.get(customer + 1).getEndWindow())) {
            //&& (arrivalTime1 <= a.latestTime.get(indexTour).get(previousPos)))	{
            isFeasible1 = true;
        }
    ...
    static void insertUnroutedCustomers(Ant a, VRPTW vrp, ArrayList<Integer> unvisitedNodes, int startIndexTour, ArrayList<Integer> startPos) {
        ...
        double arrivalTime, beginService = 0.0, newArrivalTime, newBeginService, oldBeginService, newDistance;
//        double newQuantity;
        double[] newQuantityArray;
...
//                newQuantity = a.currentQuantity.get(bestIndexTour) + reqList.get(bestCustomer + 1).getDemand();
                newQuantityArray = addDoubleArray(a.currentQuantityArray.get(bestIndexTour), reqList.get(bestCustomer + 1).getDemandArray());

//                a.currentQuantity.set(bestIndexTour, newQuantity);
                a.currentQuantityArray.set(bestIndexTour, newQuantityArray);
